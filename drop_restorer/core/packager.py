...
def installation_instructions(theme_name='wordpress-theme.zip', content_name='content.xml'):
    return ('# Установка восстановленного сайта\n\n'
        ...
        '5. Настройки → Постоянные ссылки: проверьте, что отключён Plain. При активации и импорте тема автоматически включает «Название записи», если структура была пустой; существующая непустая структура сохраняется. Нажмите «Сохранить изменения», если красивые URL возвращают 404.\n'
        '6. Проверьте все архивные пути, три казино-страницы формата `/casino/<слаг>/` и /go/ на установленном домене. Превью не проверяет правила вашего хостинга. Если остаётся Apache 404, проверьте WordPress-правила .htaccess и AllowOverride/mod_rewrite; для Nginx — передачу отсутствующих путей в index.php. Контент и метаданные казино редактируются владельцем; canonical уже задан.\n'
        ...
        'Тема устанавливается первым ZIP, страницы — через XML. При повторном импорте проверьте отсутствие дублей.\n\n'
